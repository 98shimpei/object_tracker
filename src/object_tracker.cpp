  ...
  if (!init_flag) {
    for (int i = 0; i < msg.markers.size(); i++) {
      if (msg.markers[i].id == 12) {
        ...
        int px = (int)(image.cols/2.0*std::tan((90.0-(70/2.0))/360.0*2*M_PI) * x/z) + image.cols/2.0 - 20;
        int py = (int)(image.rows/2.0*std::tan((90.0-(43/2.0))/360.0*2*M_PI) * y/z) + image.rows/2.0 - 20;
        std::cout << x << " " << y << " " << z << " " << px << " " << py << " " << image.cols << " " << image.rows << std::endl;
        rectangle_value = cv::Rect2i(px, py, 40, 40);
        tracking_image = cv::Mat(image, rectangle_value).clone();
        first_tracking_image = cv::Mat(image, rectangle_value).clone();
        is_tracking = true;
      }
